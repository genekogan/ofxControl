...
void ofApp::setup(){
    ...
    panel.addMultiSlider("position", &position, ofVec2f(500, 400), ofVec2f(ofGetWidth(), ofGetHeight()));
    panel.addColor("my color", &myColor);

    ofxControlMenu *menu1 = panel.addMenu("multi choice menu", this, &ofApp::multiChoiceMenuSelect);
    menu1->setMultipleChoice(true);
    menu1->addToggle("new york", &b1);
    menu1->addToggle("los angeles", &b2);
    menu1->addToggle("chicago", &b3);

    ofxControlMenu *menu2 = panel.addMenu("single choice menu", this, &ofApp::menuSelect);
...
{
    ofLog(OF_LOG_NOTICE, "menu selection event: " + e.toggle->getName() + " = " + ofToString(e.value));
    greeting = e.toggle->getName();
}

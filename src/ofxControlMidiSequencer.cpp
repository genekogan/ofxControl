...
ofxControlMidiSequencer::ofxControlMidiSequencer(string name) : GuiWidget(name)
{
    ...
    newEvent = new ofxControlMidiSequencerEvent(0, 0, 0, 0);
    selectedEvent = NULL;
    mouseOverEvent = NULL;
    mouseOverGrid = false;
    ...
    ofAddListener(clock.beatEvent, this, &ofxControlMidiSequencer::eventBeat);
...
void ofxControlMidiSequencer::setupKeys()
{
...
ofxControlMidiSequencer::~ofxControlMidiSequencer()
{
    for (auto m : events) {
        delete m;
    }
    events.clear();
    ofRemoveListener(clock.beatEvent, this, &ofxControlMidiSequencer::eventBeat);
}

void ofxControlMidiSequencer::setActive(bool active)
{
    this->active = active;
    active ? clock.start() : clock.stop();
}

void ofxControlMidiSequencer::setNumberBeats(int numBeats)
{
...
void ofxControlMidiSequencer::setBpm(int bpm)
{
    this->bpm = bpm;
    clock.setBpm(bpm);
}

void ofxControlMidiSequencer::setPeriod(int measures)
{
    this->measures = measures;
    setNumberBeats(measures * 16);
}

void ofxControlMidiSequencer::setNoteDisplayRange(int noteMin, int noteMax)
{
...
void ofxControlMidiSequencer::setTimeDisplayRange(float start, float end)
{
...
void ofxControlMidiSequencer::setMidiEventRectangle(ofxControlMidiSequencerEvent *event)
{
...
ofxControlMidiSequencer::ofxControlMidiSequencerEvent * ofxControlMidiSequencer::addMidiEvent(int note, int velocity, int start, int end)
{
    ofxControlMidiSequencerEvent *midiEvent = new ofxControlMidiSequencerEvent(note, velocity, start, end);
...
void ofxControlMidiSequencer::repositionMidiEvent(ofxControlMidiSequencerEvent* event)
{
    for (int b = 0; b < midiOnEvents.size(); b++)
    {
        vector<ofxControlMidiSequencerEvent*>::iterator it1 = midiOnEvents[b].begin();
        vector<ofxControlMidiSequencerEvent*>::iterator it2 = midiOffEvents[b].begin();
...
void ofxControlMidiSequencer::removeEvent(ofxControlMidiSequencerEvent *event, bool toDelete)
{
    for (int b = 0; b < midiOnEvents.size(); b++)
    {
        vector<ofxControlMidiSequencerEvent*>::iterator it = midiOnEvents[b].begin();
    ...
    {
        vector<ofxControlMidiSequencerEvent*>::iterator it = midiOffEvents[b].begin();
    ...
    vector<ofxControlMidiSequencerEvent*>::iterator it = events.begin();
    while (it != events.end())
    {
        if (*it == event) {
            events.erase(it);
...
void ofxControlMidiSequencer::removeSelectedEvents()
{
    vector<ofxControlMidiSequencerEvent*> eventsToRemove;
    for (auto m : events)
    {
    ...
    vector<ofxControlMidiSequencerEvent*>::iterator itr = eventsToRemove.begin();
...
void ofxControlMidiSequencer::eventBeat()
{
...
void ofxControlMidiSequencer::draw()
{
...
void ofxControlMidiSequencer::drawGrid()
{
    if (active)
    {
...
void ofxControlMidiSequencer::drawKeyboard()
{
    ofSetColor(0);
    ofRect(rectKeyboard);
...
void ofxControlMidiSequencer::drawVelocity()
{
    ofSetColor(255);
    ofRect(rectVelocity);
...
void ofxControlMidiSequencer::drawToolbar()
{
    // draw toolbar
    ofSetColor(0, 0, 200);
    ofRect(rectToolbar);
...
bool ofxControlMidiSequencer::keyPressed(int key)
{
...
void ofxControlMidiSequencer::setupGuiPositions()
{
